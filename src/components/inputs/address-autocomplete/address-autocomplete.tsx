    ...
    const fetchPlaceId = async () => {
      if (!!address.formattedAddress && !selectedPlaceId) {
        try {
          ...
          if (result.error) {
            setError(
              (result.error as string) ?? "Error trying to find placeid",
            );
...
function AddressAutoCompleteInput(props: CommonProps) {
  ...
  return (
    ...
    >
      <div className="bg-background ring-offset-background focus-within:ring-ring flex w-full items-center justify-between rounded-lg border text-sm focus-within:ring-2 focus-within:ring-offset-2">
      ...
      {isOpen && (
        <div className="animate-in fade-in-0 zoom-in-95 relative h-auto">
          <CommandList>
            <div className="absolute top-1.5 z-50 w-full">
              <CommandGroup className="bg-background relative z-50 h-auto min-w-[8rem] overflow-hidden rounded-md border shadow-md">
                {isLoading ? (
                    ...
                    {predictions.map(
                      (prediction: {
                        ...
                        <CommandPrimitive.Item
                          ...
                          className="hover:bg-accent hover:text-accent-foreground aria-selected:bg-accent aria-selected:text-accent-foreground flex h-max cursor-pointer flex-col items-start gap-0.5 rounded-md p-2 px-3 select-text"
