  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch("/api/join/verify-code", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ invitationCode, type: "guest" }),
      });
      ...
      <div className="border-border bg-muted/30 mt-10 rounded-xl border p-6">
        ...
        <ul className="text-muted-foreground mt-4 space-y-3 text-sm">
          <li className="flex gap-3">
            ...
            <span>
              Discover unique, locally crafted products and services by our
              artisans
            </span>
          </li>
          <li className="flex gap-3">
            ...
            <span>
              Gain access to new and exclusive tools used by our artisans
            </span>
          </li>
          <li className="flex gap-3">
            <span className="bg-primary/10 text-primary mt-1 flex size-5 shrink-0 items-center justify-center rounded-full text-xs">
            ...
            <span>
              Offer your feedback and suggestions to improve the platform
            </span>
          </li>
        </ul>
      </div>
