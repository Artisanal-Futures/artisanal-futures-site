...
import { useDriversStore } from '~/app/tools/solidarity-pathways/_stores/use-drivers-store'
...
export const useReadDriver = () => {
  ...
  const getDriverById = (vehicleId: BundleId): BundleResult => {
    ...
    if (isUserAllowedToSaveToDepot) return checkIfDriverExistsInDepot(vehicleId)
  ...
  const getVehicleById = (vehicleId: BundleId): BundleResult => {
    ...
    if (isUserAllowedToSaveToDepot) return checkIfDriverExistsInRoute(vehicleId)
