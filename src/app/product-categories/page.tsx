          ...
          {categories.map((category) => (
            <div key={category.id}>
              <h2 className="mb-4 text-3xl font-bold tracking-tight">
                <Link href={`/product-category/${category.name.toLowerCase()}`} className="hover:underline">
                  {category.name}
                </Link>
              </h2>
              {category.children.length > 0 && (
                <div className="grid grid-cols-2 gap-4 sm:grid-cols-3 md:grid-cols-4">
                  {category.children.map((sub) => (
                    <Link
                      key={sub.id}
                      href={`/product-category/${category.name.toLowerCase()}?subcategory=${sub.name.toLowerCase()}`}
