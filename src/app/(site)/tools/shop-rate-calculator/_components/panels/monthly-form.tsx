...
export function MonthlyCostForm() {
  ...
  const onSubmit = (data: MonthlyFormValues) => {
    ...
    toast.info("Monthly expenses updated");
  };
            ...
            <ScrollArea className="bg-card col-span-full max-h-96 rounded-lg border p-4 shadow-sm">
              {fields.map((field, index) => {
                return (
                  <div
                    key={field.id}
                    className="bg-background hover:bg-accent/10 mb-4 flex w-full items-end justify-between gap-6 rounded-md p-4 shadow-sm transition-colors sm:col-span-full"
                  >
                    <FormField
                      control={form.control}
